...
static int mxc_allocate_frame_buf(cam_data *cam, int count)
{
	...
	pr_err("*** <B2>count=%d width=%d sizeimage=%d \n",
			count, cam->v2f.fmt.pix.width, cam->v2f.fmt.pix.sizeimage);
...
{
	int retval,pix = 0;                      //JAD added pix
	int size = 0;
	int bytesperline = 0;
	int *width, *height;
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		*height -= *height % 8;
		...
		pix=f->fmt.pix.pixelformat;												 //JAD
		pr_err("*** <D2>f->fmt.pix.pixelformat = %i \n",pix);                    //JAD
		f->fmt.pix.pixelformat=V4L2_PIX_FMT_YUV420;                              //JAD
		...
		pr_err("*** <B3>mxc_v4l2_s_fmt sizeimage=%d bytesperline=%d\n",
			f->fmt.pix.sizeimage,f->fmt.pix.bytesperline);
		pr_err("*** <B4>pixelformat=%x\n",f->fmt.pix.pixelformat);
...
static int init_camera_struct(cam_data *cam, struct platform_device *pdev)
{
	...
	*(cam->video_dev) = mxc_v4l_template;
	...
	cam->v2f.fmt.pix.pixelformat = V4L2_PIX_FMT_YUV422P;   //JAD 1. was V4L2_PIX_FMT_YUV420
	cam->win.w.width = 160;
	...
	cam->fmt_in  = IPU_PIX_FMT_UYVY;       //JAD 3. was IPU_PIX_FMT_UYVY
	cam->fmt_out = IPU_PIX_FMT_YUV422P;    //JAD 2. was IPU_PIX_FMT_UYVY or IPU_PIX_FMT_YUV422P
	cam->frame_out_cnt = 0;
	g_callback_count = 0;
